...
- **Both inline AND TX reference in training payload.** Each `dataset_inputs` entry retains its inlined metadata (digest, schema_hash, etc.) AND gains an `anchor_tx` field. Reasoning: each event verifies independently (training's SoT catches inline-metadata mutations even if the dataset proof is unreachable; dataset's SoT catches dataset-proof mutations independently). Slight redundancy, much more robust verification.

  Distinction worth keeping in mind for future readers: chain walking (an auditor following `anchor_tx` from training to dataset to prove lineage) and single-event integrity verification (does this event's signed bytes still match the live MLflow state?) are different operations. Chain walking is always a feature — that's why `anchor_tx` references exist. Inline metadata exists so a single event's *integrity* can be checked without cascading dependencies on every referenced event being reachable. Lineage walks the chain; integrity does not need to.
